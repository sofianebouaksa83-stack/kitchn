...
export default function RecipeDisplayMobile({
  ...
  // ✅ sections accordion (ouvertes par défaut sur mobile pour une lecture rapide)
  ...
  async function load() {
    ...
    try {
      ...
      // ✅ sections ouvertes par défaut : la bottom sheet devient la fiche rapide de service
      const opened: Record<string, boolean> = {};
      for (const s of sortedSections) opened[s.id] = true;
  ...
  function increaseMultiplier() {
    const currentMultiplier = Math.max(1, Math.round(coefficient || 1));
    setCrossHave("");
    setServings(baseServings * (currentMultiplier + 1));
  }

  function decreaseMultiplier() {
    const currentMultiplier = Math.max(1, Math.round(coefficient || 1));
    setCrossHave("");
    setServings(baseServings * Math.max(1, currentMultiplier - 1));
  }
  ...
  const allergensText = useMemo(() => safeText(recipe?.allergens), [recipe?.allergens]);
  ...
  const unlinkedIngredients = useMemo(() => {
    const linkedIds = new Set(links.map((link) => link.ingredient_id));
